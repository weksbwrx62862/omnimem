...
class FileLockManager:
    ...
    def acquire(self, timeout: float = 5.0, exclusive: bool = True) -> bool:
        ...
        lock_type = fcntl.LOCK_EX if exclusive else fcntl.LOCK_SH  # type: ignore[union-attr]
...
class SyncEngine:
    ...
    def __init__(self, data_dir: Path, config: Optional[SyncConfig] = None):
        self._data_dir = data_dir
        self._config = config or SyncConfig()
        self._active = self._config.mode != "none"

        # 文件锁(单主机多进程)
        self._file_lock: Optional[FileLockManager] = None
