...
class _CachedEmbeddingFunction:
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2", cache_path: Optional[Path] = None):
    ...
    def __call__(self, input: List[str]) -> List[List[float]]:
        ...
        if to_encode:
            ...
            with self._lock:
                for (orig_idx, text_hash), emb in zip(to_encode_idx, embeddings):
                    vec = emb.tolist()
...
class VectorRetriever:
    ...
    def __init__(self, backend: str = "chromadb", data_dir: Path = None):
    ...
    def _ensure_initialized(self) -> None:
        ...
        try:
            ...
            logger.debug("ChromaDB collection initialized: %d documents",
    ...
    def add_batch(self, documents: List[Dict[str, Any]]) -> None:
        ...
        for doc in documents:
            ...
            if not content or not memory_id:
                continue
            meta = {k: str(v) for k, v in doc.items() if k not in ("content", "memory_id") and v is not None}
    ...
    def _add_single(self, content: str, memory_id: str, metadata: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if len(content) > self._CHUNK_SIZE:
                chunks = self._split_chunks(content, self._CHUNK_SIZE, self._CHUNK_OVERLAP)
                # ★ 稳定 chunk ID：基于内容哈希，避免重复索引导致存储膨胀
                ids = [f"{memory_id}_chunk{hashlib.md5(chunk.encode()).hexdigest()[:8]}" for chunk in chunks]
                metas = [dict(meta, _parent_id=memory_id, _chunk_idx=str(i))
    ...
    def _merge_chunk_results(results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for parent_id, chunks in parent_chunks.items():
            ...
            has_star_chunk = (
                len(chunks) >= 2 and
                best_score > avg_score + score_std and  # 显著高于平均
                best_score > 0.6  # 自身也是高相关
            )
            ...
            if has_star_chunk:
                ...
                for chunk in chunks:
                    chunk_entry = {k: v for k, v in chunk.items()
                                  if k not in ("_parent_id", "_chunk_idx")}
                ...
                merged = {k: v for k, v in chunks[0].items()
