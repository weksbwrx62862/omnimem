...
# 运行时中文词词典（从外部 JSON 加载，可被用户自定义覆盖）
_COMMON_ZH_WORDS: set[str] = set()


def _load_common_zh_words() -> set[str]:
    """从外部 JSON 加载中文词词典，合并默认值与用户自定义。

    加载策略：
      1. 以 _DEFAULT_COMMON_ZH_WORDS 为基础
      2. 尝试从 config/zh_words.json 加载并覆盖默认值
      3. 加载失败时回退到默认值并记录 warning
    """
    result = set(_DEFAULT_COMMON_ZH_WORDS)
    ...
    try:
        ...
        logger.debug("zh_words.json not found at %s, using defaults", config_path)
    except Exception:
        logger.warning("Failed to load zh_words.json from %s, using defaults", config_path)
    return result
